...
int main(int argc, char *argv[]){
    ...
    infd = client_connect(hostname, portnumber);
    outfd = infd;
    
    char strid[14];
    sprintf(strid, "ID=%d\n", id);
    write(logfd, strid, strlen(strid));
    write(outfd, strid, strlen(strid));
    
    //initialize snesors
    sensor_init();
    
    //setup up polling
    struct pollfd pollStdin = {infd, POLLIN, 0};
